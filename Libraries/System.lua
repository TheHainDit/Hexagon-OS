...
function system.updateDesktop()
	...
	local Hexagon-OSContextMenu = desktopMenu:addContextMenuItem("Hexagon OS", 0x000000)
	Hexagon-OSContextMenu:addItem(localization.aboutSystem).onTouch = function()
	...
	Hexagon-OSContextMenu:addItem(localization.updates).onTouch = function()
		system.execute(paths.system.applicationAppMarket, "updates")
	end

	Hexagon-OSContextMenu:addSeparator()

	Hexagon-OSContextMenu:addItem(localization.logout).onTouch = function()
		system.authorize()
	end

	Hexagon-OSContextMenu:addItem(localization.reboot).onTouch = function()
		require("Network").broadcastComputerState(false)
		computer.shutdown(true)
	end

	Hexagon-OSContextMenu:addItem(localization.shutdown).onTouch = function()
		require("Network").broadcastComputerState(false)
		computer.shutdown()
	end
